...
## project overview
- `nautobot` is a rust client for the nautobot rest api.
- workspace:
  - `crates/nautobot`: public client crate.
  - `crates/nautobot-openapi`: generated bindings. avoid manual edits.
  - `crates/nautobot-cli`: cli for manual testing.
...
- `crates/nautobot-openapi/src/`: generated api bindings.
- `crates/nautobot-cli/src/main.rs`: cli entrypoint.
...
generated bindings live under `crates/nautobot-openapi`. do not hand-edit.

before running codegen, confirm with the user that nautobot is running.

once nautobot is running locally:
1. fetch schema:
   ```bash
...
## agent checklist
- confirm nautobot is running and reachable.
- do not start nautobot containers; the user manages that.
- ask for the base url if not `http://localhost:8000`.
- confirm `NAUTOBOT_TOKEN` is set before smoke tests.

## api wrapper patterns

when adding a new endpoint wrapper in `crates/nautobot/src/{dcim,ipam}/`:
...
- update `crates/nautobot/docs/client.md` for client behavior changes.

## testing guidance
- add unit tests for new helper logic.
- prefer lightweight tests in `crates/nautobot/src/*`.
- avoid network calls unless explicitly requested.
- smoke tests live in `crates/nautobot/tests/smoke_local.rs` and are ignored by default.
