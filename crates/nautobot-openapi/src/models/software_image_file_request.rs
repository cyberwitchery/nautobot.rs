/*
 * API Documentation
 *
 * Source of truth and network automation platform
 *
 * The version of the OpenAPI document: 3.1.6 (3.1)
 *
 * Generated by: https://openapi-generator.tech
 */

/// SoftwareImageFileRequest : Base class to use for serializers based on OrganizationalModel or PrimaryModel.  Can also be used for models derived from BaseModel, so long as they support custom fields, notes, and relationships.

#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct SoftwareImageFileRequest {
    #[serde(rename = "id", skip_serializing_if = "Option::is_none")]
    pub id: Option<uuid::Uuid>,
    #[serde(rename = "image_file_name")]
    pub image_file_name: String,
    #[serde(
        rename = "image_file_checksum",
        skip_serializing_if = "Option::is_none"
    )]
    pub image_file_checksum: Option<String>,
    #[serde(rename = "hashing_algorithm", skip_serializing_if = "Option::is_none")]
    pub hashing_algorithm:
        Option<Box<crate::models::BulkWritableSoftwareImageFileRequestHashingAlgorithm>>,
    /// Image file size in bytes
    #[serde(
        rename = "image_file_size",
        default,
        with = "::serde_with::rust::double_option",
        skip_serializing_if = "Option::is_none"
    )]
    pub image_file_size: Option<Option<i64>>,
    #[serde(rename = "download_url", skip_serializing_if = "Option::is_none")]
    pub download_url: Option<String>,
    /// Is the default image for this software version
    #[serde(rename = "default_image", skip_serializing_if = "Option::is_none")]
    pub default_image: Option<bool>,
    #[serde(rename = "software_version")]
    pub software_version: Box<crate::models::ApprovalWorkflowStageResponseApprovalWorkflowStage>,
    #[serde(
        rename = "external_integration",
        default,
        with = "::serde_with::rust::double_option",
        skip_serializing_if = "Option::is_none"
    )]
    pub external_integration: Option<Option<Box<crate::models::ApprovalWorkflowUser>>>,
    #[serde(rename = "status")]
    pub status: Box<crate::models::ApprovalWorkflowStageResponseApprovalWorkflowStage>,
    #[serde(rename = "custom_fields", skip_serializing_if = "Option::is_none")]
    pub custom_fields: Option<::std::collections::HashMap<String, serde_json::Value>>,
    #[serde(rename = "relationships", skip_serializing_if = "Option::is_none")]
    pub relationships: Option<
        ::std::collections::HashMap<
            String,
            crate::models::ApprovalWorkflowDefinitionRequestRelationshipsValue,
        >,
    >,
    #[serde(rename = "tags", skip_serializing_if = "Option::is_none")]
    pub tags: Option<Vec<crate::models::ApprovalWorkflowStageResponseApprovalWorkflowStage>>,
}

impl SoftwareImageFileRequest {
    /// Base class to use for serializers based on OrganizationalModel or PrimaryModel.  Can also be used for models derived from BaseModel, so long as they support custom fields, notes, and relationships.
    pub fn new(
        image_file_name: String,
        software_version: crate::models::ApprovalWorkflowStageResponseApprovalWorkflowStage,
        status: crate::models::ApprovalWorkflowStageResponseApprovalWorkflowStage,
    ) -> SoftwareImageFileRequest {
        SoftwareImageFileRequest {
            id: None,
            image_file_name,
            image_file_checksum: None,
            hashing_algorithm: None,
            image_file_size: None,
            download_url: None,
            default_image: None,
            software_version: Box::new(software_version),
            external_integration: None,
            status: Box::new(status),
            custom_fields: None,
            relationships: None,
            tags: None,
        }
    }
}
