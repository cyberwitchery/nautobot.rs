# nautobot api generation scripts

scripts for generating the nautobot openapi client.

## workflow

### 1. start nautobot

use the official nautobot-docker setup or similar.

wait until nautobot is ready.
...
this writes to `crates/nautobot-openapi/src/`.
...
## update for a new nautobot version

1. update the nautobot-docker image tag
...
```bash
curl -fsSL https://nautobot.example.com/api/schema/?format=json \
...
notes:
- `nautobot-openapi` must be published before `nautobot`
- the script packages `nautobot` with `--no-verify` because the dependency is not on crates.io yet
...
- `cargo llvm-cov --workspace --all-features --ignore-filename-regex 'crates/nautobot-openapi' --fail-under-lines 75`
- `./scripts/run_smoke.sh`
- `./scripts/run_cli_smoke.sh`

requirements:
- `cargo-llvm-cov` installed
- `NAUTOBOT_TOKEN` set for smoke tests
